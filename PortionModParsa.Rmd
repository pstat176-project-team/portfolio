...
#"COST","NFLX", "AMZN", "BRK-A", "DPZ", "ORLY"

```{R, echo = FALSE, include = FALSE}
#Step 7: read daily CLOSE prices for all assets
tickers = c("NVDA", "SIMO", "AVGO", "MCHP", "AMD", "SMTC", "WOLF", "VECO", "AMAT", "KLAC", "ASML","COST", "NFLX", "AMZN", "BRK-B", "DPZ", "ORLY" )
```
```{r, echo = FALSE, include = FALSE}
# Expense Ratios:
...
rt_daily_init <- adjust_returns(rt_daily_init, fees)
```
...
if (TRUE) {
  # Pick risk profile 2
  tgt_pro_2 <- as.numeric(rt_tgt[index(rt_daily[1,]), 3])
...
sum_pos <- sum(portRisk3.wts[1,][portRisk3.wts[1,] >= 0])
if (sum_pos > 1.2){
  portRisk3.wts[1,][portRisk3.wts[1,] >= 0] <- portRisk3.wts[1,][portRisk3.wts[1,] >= 0]/sum_pos*1.2
  sum_neg <- sum(portRisk3.wts[1,][portRisk3.wts[1,] < 0])
  portRisk3.wts[1,][portRisk3.wts[1,] < 0] <- portRisk3.wts[1,][portRisk3.wts[1,] < 0]*0.2/(-sum_neg)
}
...
for (rper in 2:rebal_freq) {
  ...
  sum_pos <- sum(portRisk3.wts[rper,][portRisk3.wts[rper,] >= 0])
  if (sum_pos > 1.2){
    portRisk3.wts[rper,][portRisk3.wts[rper,] >= 0] <- portRisk3.wts[rper,][portRisk3.wts[rper,] >= 0]/sum_pos*1.2
    sum_neg <- sum(portRisk3.wts[rper,][portRisk3.wts[rper,] < 0])
    portRisk3.wts[rper,][portRisk3.wts[rper,] < 0] <- portRisk3.wts[rper,][portRisk3.wts[rper,] < 0]*0.2/(-sum_neg)
  }
...
Yr_2015Return <- 1+(port_val[5] - port_val[1])/port_val[1]
Yr_2016Return <- (1+(port_val[9] - port_val[1])/port_val[1])^(1/2)
Yr_2017Return <- (1+(port_val[13] - port_val[1])/port_val[1])^(1/3)
Yr_2018Return <- (1+(port_val[17] - port_val[1])/port_val[1])^(1/4)
Yr_2019Return <- (1+(port_val[21] - port_val[1])/port_val[1])^(1/5)
Yr_2020Return <- (1+(port_val[25] - port_val[1])/port_val[1])^(1/6)
Yr_2015Return
Yr_2016Return
Yr_2017Return
Yr_2018Return
...
```{r}
hist(rt_ptf, main = "Histogram of Portfolio Returns, MS", xlab = "Returns", ylab = "Density")
hist(rt_ptfGMV, main = "Histogram of Portfolio Returns, GMV", xlab = "Returns", ylab = "Density")
hist(rt_ptfR3, main = "Histogram of Portfolio Returns, Target", xlab = "Returns", ylab = "Density")
```
```{r}
...
edf_manual <- data.frame(Returns = unique_rt, CumulativeProbability = cum_prob)
# par(mfrow = c(1,2), mar = c(3,2,2,3))
plot(unique_rt, cum_prob, type = "l", main = "Empirical Distribution Function (EDF) of Target Portfolio Returns",xlab = "Returns", ylab = "EDF")
```
...
# Density plot with 5% quantile line
plot(density(rt_ptfR3[, 1]), main = "Kernel Density of Target Portfolio Returns", 
     xlab = "Returns", ylab = "Density")
abline(v = quantile(rt_ptfGMV, probs = 0.05), col = "red", lty = "dashed", lwd = 2)
legend("topright", legend = c("Density", "5% Quantile"), 
       col = c("black", "red"), lty = c("solid", "dashed"), lwd = c(1, 2), cex = 0.8)
...
plot(daily_MDD, type = "l", main = "Maximum Draw-Down over the last 20 days, Target", ylab = "MDD")
```
...
plot(daily_Omega, type = "l", main = "Omega Ratio over the last 20 days Target", ylab = "Omega")
```
...
returns <- ROC(Cl(to.quarterly(SPY)))

# Plot the quarterly returns
plot(returns, type = "l", col = "blue", main = "SPY Quarterly Returns",
...
# Fetch SPY data from Yahoo Finance
getSymbols("ULST", from = start_date, to = end_date)

# Calculate quarterly returns
returns <- ROC(Cl(to.quarterly(ULST)))

# Plot the quarterly returns
plot(returns, type = "l", col = "blue", main = "ULST Quarterly Returns",
     xlab = "Date", ylab = "Returns")
```
...
```{r}
benchmark_prices <- Ad(getSymbols("VOO", from = "2015-01-01", to = "2020-12-31", 
                                  auto.assign = FALSE))
...
plot(treynor, type = "l", main = "Treynor Coefficient over the last 20 days Target", ylab = "Treynor Coefficient")
```