---
title: "PSTAT176Project_finalcodeversion"
author: "JiayueChen"
date: "2023-05-16"
...
# expense ratio
```{r}
#expense ratios function for ETF fee
...
# adjusted return
```{r}
#adjust returns that calculate the return after deduction of fees
...
cpi <- fredr(
  ...
  frequency = "m" # interested in comparing return with monthly CPI index
)
```
...
```
# Step 2 - Target Return for Moderate Risk
```{r}
# Inflation rate
ts_inflation <- na.omit(diff(log(ts_cpi))) #same as Return.calculate(ts_cpi, method = "log")
...
head(ts_med_returns)
```
# Step 3&4 - Daily Close Price for DIA,ULST, and SPY
```{r}
start_date = "2014-12-31" # adjust this to avoid NA
end_date = "2020-12-31"
...
# Step 5 - Daily Log-return from step3&4
```{r}
SPY_close_daily_log <- Return.calculate(SPY_all, method = "log")
DIA_close_daily_log <- Return.calculate(DIA_all, method = "log")
ULST_close_daily_log <- Return.calculate(ULST_all, method = "log")

head(SPY_close_daily_log)
head(DIA_close_daily_log)
head(ULST_close_daily_log)
tail(SPY_close_daily_log)
tail(DIA_close_daily_log)
tail(ULST_close_daily_log)
```
...
adj_SPY_close_daily <- adjust_returns(SPY_close_daily_log, SPYfee)
adj_DIA_close_daily <- adjust_returns(DIA_close_daily_log, DIAfee)
adj_ULST_close_daily <- adjust_returns(ULST_close_daily_log, ULSTfee)
```

# Step 6 - SD of SPY,DIA, and ULST
```{r}
sd_window <- 126 # 6 months of trading days
sd_daily_SPY <- na.omit(rollapply(SPY_close_daily_log, width = sd_window, FUN = sd, align = "right", fill = NA))
sd_daily_DIA <- na.omit(rollapply(DIA_close_daily_log, width = sd_window, FUN = sd, align = "right", fill = NA))
sd_daily_ULST <- na.omit(rollapply(ULST_close_daily_log, width = sd_window, FUN = sd, align = "right", fill = NA))
Tolerable_stdDev <- -8* sd_daily_SPY #note we can't have a loss that drops below the given limit on a single day
Tolerable_stdDev
```

# Step 7 - Daily Close price of portfolio assets
Costco(COST)
Walmart(WMT)
Pfizer(PFE)
Johnson & Johnson(JNJ)
...
```{r}
tickers <- c("BRK-B","COST","WMT","PFE","JNJ","LLY","ED","AWK","MSFT","AAPL","PANW","AKAM","NOW","BND")
# Expense Ratios for ETFs
BNDfee <- get_etf_data("BIV")

fees <- c(BNDfee)
for (i in seq(length(tickers) - 1)){
  fees <- c(fees, 0)
}
fees
...
# Step 8 - Daily log return of Step 7
```{r}
rt_daily <- na.omit(Return.calculate(pr_daily, method = "log"))
fee_adj_rt_daily <- adjust_returns(rt_daily, fees)
head(fee_adj_rt_daily)
```

# Step 9 - Decide window of time you will calculate your historical covariance for log-returns

Choose historical window of 6 months.

# Step 10 - Decide portfolio rebalance schedule

Rebalancing quarterly

# Step 11 - Calculate the covariance matrix of your assets log-returns at each rebalance data in the year, based on the decided look-back window
Then we graph the efficient frontier for 6 months of historical covariance. This particular efficient frontier is computed to show the distribution of assets at 2015-01-02.
```{r}
...
# Step 12 - Calculate Global Minimum Variance portfolio at each rebalance date using your sample estimated covariance matrix
# Step 13 - Calculate Max-Sharpe Portfolio at each rebalance date using your sample estimated covariance matrix.
Target Return
```{r}
# Load packages
library(quadprog)  
...
library(lubridate)
library(fredr)
# To install NMOF run the following two lines
# install.packages("remotes")
# remotes::install_github("NMOF/NMOF")
library(NMOF)
```
```{r}
fredr_set_key("ab27202799a83cd2409a1c45fac154c1")
...
Portfolio
```{r}
tickers <- c("WMT","JNJ","AWK","MSFT","AAPL","AMZN","TSLA","V","NFLX")
...
plot(prices$NFLX,main="BRK-B",theme = 'white')
# plot(prices$COST,main="COST",theme = 'white')
plot(prices$WMT,main="WMT",theme = 'white')
# plot(prices$PFE,main="PFE",theme = 'white')
plot(prices$JNJ, main="JNJ",theme = 'white')
# plot(prices$ED, main="ED",theme = 'white')
plot(prices$AWK, main="AWK",theme = 'white')
plot(prices$MSFT, main="MSFT",theme = 'white')
plot(prices$AAPL, main="AAPL",theme = 'white')
plot(prices$MA, main="AKAM",theme = 'white')
plot(prices$TSLA, main="NOW",theme = 'white')
# plot(prices$BND, main="BND",theme = 'white')
```
Portfolio Efficient Frontier
```{r}
# Computing the robust mean returns and variance-covariance matrix
...
if (FALSE) {
  # Pick risk profile 2
  tgt_pro_2 <- as.numeric(rt_tgt[index(rt_daily[1,]), 2])

  # Efficient portfolio with target return for risk profile 2
  efficient_portfolio <- tseries::portfolio.optim(x = rt_daily_init, pm = tgt_pro_2, 
...
if (FALSE) {
  ...
  port.wts[1,] <- t(invSigma %*% ones / sum(invSigma %*% ones))
}
...
# Specific target risk
  ...
  tgt_pro_2 <- mean(as.numeric(rt_tgt[paste0(end_date_loop + days(1), "/", 
                                             end_date_loop + days(7)), 2]))

  # Efficient portfolio with target return = mean of returns
  efficient_portfolio <- tseries::portfolio.optim(x = window_returns, pm = tgt_pro_2, 
...
# Step 14 - Calculate Efficient Frontier at each portfolio rebalance date and plot each months efficient frontier on the risk return plot for the year (if monthly rebalance there would be 12 frontiers on the plot – one for each month)
same in step 11

# Step 15 - Select your target portfolio(s) weights at the start of each rebalance date (consider the constraints outlined in the speciation’s) and assume you hold those portfolio weights fixed until next rebalance date, calculate the daily portfolio returns based on your selected portfolios.

# Step 16 - Assess your portfolio(s) performance over each rebalance period and summarise this performance visually with relevant plots.
...
start_date = "2014-12-31"  #to deal with nas in rolling returns. 
...
```{r}
rt_SPY_close_daily<-na.omit(rt_SPY_close_daily)
(sum(rt_SPY_close_daily)/1510)*1510
rt_DIA_close_daily_time<-na.omit(rt_DIA_close_daily_time)
(sum(rt_DIA_close_daily_time)/1510)*1510
```
...
quarterly_returns_SPY <- to.quarterly(rt_adj_SPY_close_daily_time)
quarterly_returns_DIA <- to.quarterly(rt_adj_DIA_close_daily_time)
quarterly_returns_ULST <- to.quarterly(rt_adj_ULST_close_daily_time)
```

```{r, echo = FALSE}
plot(quarterly_returns_SPY$rt_adj_SPY_close_daily_time.Close, main = "Quarterly Returns of SPY", ylab = "Returns")
plot(quarterly_returns_DIA$rt_adj_DIA_close_daily_time.Close, main = "Quarterly Returns of DIA", ylab = "Returns")
plot(quarterly_returns_ULST$rt_adj_ULST_close_daily_time.Close, main = "Quarterly Returns of ULST", ylab = "Returns")
```
...
```{r, message = FALSE, echo = FALSE}
# Need to include Risk profiles 2 though name is 3, it's risk profile 2
set.seed(1)
...
rt_ptfGMV2015 <- rt_ptfGMV["/2016-01-01"]
(sum(rt_ptfGMV2015)/length(rt_ptfGMV2015)) * 252

rt_ptfGMV2016 <- rt_ptfGMV["/2017-01-01"]
(sum(rt_ptfGMV2016)/length(rt_ptfGMV2016)) * 252

rt_ptfGMV2017 <- rt_ptfGMV["/2018-01-01"]
(sum(rt_ptfGMV2017)/length(rt_ptfGMV2017)) * 251

rt_ptfGMV2018 <- rt_ptfGMV["/2019-01-01"]
(sum(rt_ptfGMV2018)/length(rt_ptfGMV2018)) * 251

rt_ptfGMV2019 <- rt_ptfGMV["/2020-01-01"]
(sum(rt_ptfGMV2019)/length(rt_ptfGMV2019)) * 252

rt_ptfGMV2020 <- rt_ptfGMV["/2021-01-01"]
(sum(rt_ptfGMV2020)/length(rt_ptfGMV2020)) * 253
```
...
hist(rt_ptfR3, main = "Histogram of Portfolio Returns", xlab = "Returns", ylab = "Density")
...
unique_rt <- sort(unique(rt_ptfR3))
n <- length(unique_rt)
cum_prob <- sapply(unique_rt, function(x) sum(unique_rt <= x) / n)
edf_manual <- data.frame(Returns = unique_rt, CumulativeProbability = cum_prob)
# par(mfrow = c(1,2), mar = c(3,2,2,3))
plot(unique_rt, cum_prob, type = "l", main = "Empirical Distribution Function (EDF) of Target Return Portfolio",xlab = "Returns", ylab = "EDF")
...
# Density plot with 5% quantile line
plot(density(rt_ptfR3[, 1]), main = "Kernel Density of Target Return Portfolio", 
     xlab = "Returns", ylab = "Density")
abline(v = quantile(rt_ptfGMV, probs = 0.05), col = "red", lty = "dashed", lwd = 2)
legend("topright", legend = c("Density", "5% Quantile"), 
       col = c("black", "red"), lty = c("solid", "dashed"), lwd = c(1, 2), cex = 0.8)
...
for (i in 1:(nrow(rt_ptfR3) - 20)) {
  window_returns <- rt_ptfR3[i:(i + 19), ]
  daily_MDD <- c(daily_MDD, maxDrawdown(window_returns))
}
plot(daily_MDD, type = "l", main = "Maximum Draw-Down over the last 20 days, Target Return Portfolio", ylab = "MDD")
...
plot(daily_Omega, type = "l", main = "Omega Ratio over the last 20 days Target Return Portfolio", ylab = "Omega")
...
plot(daily_Sortino, type = "l", main = "Sortino Ratio over the last 20 days Target Return Portfolio", ylab = "Sortino Ratio")
...
plot(daily_Sortino, type = "l", main = "Sortino Ratio over the last 20 days MS", ylab = "Sortino Ratio")
...
plot(daily_Sortino, type = "l", main = "Sortino Ratio over the last 20 days GMV", ylab = "Sortino Ratio")
```
```{r}
qqnorm(rt_ptf)
qqline(rt_ptf)
...
plot(DivD, type = "l", main = "Portfolio Diversification Delta over the last 20 days Target", ylab = "Diversification Delta")
```
...
```{r}
benchmark_prices <- Ad(getSymbols("^GSPC", from = "2015-01-01", to = "2020-12-31", 
                                  auto.assign = FALSE))
...
for (i in 1:(nrow(rt_ptf) - 20)) {
  ...
  window_rt_ptf <- na.omit(window_rt_ptf)
  window_rt_ben <- na.omit(window_rt_ben)


  window_rt_ptf <- as.numeric(window_rt_ptf)
  window_rt_ben <- as.numeric(window_rt_ben)


  CovMCD <- covRob(cbind(window_rt_ptf, window_rt_ben), estim = "mcd", quan = .95, ntrial = 10000)
  # cov(window_rt_ptf, window_rt_ben)/(sd(window_rt_ben)^2)
  betas <- c(betas, cov(window_rt_ptf, window_rt_ben)/(sd(window_rt_ben)^2))
  betasMCD <- c(betas, CovMCD$cov[1,2]/CovMCD$cov[2,2])
}
plot(1:(length(betas)), betas, type = "l", col = "blue", main = "Portfolio Beta over the last 20 days MS", xlab = "Index", ylab = "Beta")
...
plot(1:(length(betas)), betas, type = "l", col = "blue", main = "Portfolio Beta over the last 20 days Target", xlab = "Index", ylab = "Beta")
lines(1:(length(betasMCD)), betasMCD, type = "l", col = "red")
legend("topright", legend=c("Regular", "MCD"), col=c("blue", "red"), lty=1)
```
...
betasMCD <- NULL
for (i in 1:(nrow(rt_ptfGMV) - 20)) {
  window_rt_ptf <- rt_ptfGMV[i:(i + 19), ]
  window_rt_ben <- benchmark_returns[i:(i + 19), ]
...
plot(1:(length(betas)), betas, type = "l", col = "blue", main = "Portfolio Beta over the last 20 days GMV", xlab = "Index", ylab = "Beta")
lines(1:(length(betasMCD)), betasMCD, type = "l", col = "red")
legend("topright", legend=c("Regular", "MCD"), col=c("blue", "red"), lty=1)
```
...
plot(treynor, type = "l", main = "Treynor Coefficient over the last 20 days MS", ylab = "Treynor Coefficient")
mean(treynor)
```
...
plot(treynor, type = "l", main = "Treynor Coefficient over the last 20 days GMV", ylab = "Treynor Coefficient")
mean(treynor)
```
...
plot(treynor, type = "l", main = "Treynor Coefficient over the last 20 days Target Return Portfolio", ylab = "Treynor Coefficient")
mean(treynor)
```
